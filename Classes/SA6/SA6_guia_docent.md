...
> ⏱️ **Marge:** el temps efectiu real és ~100' (arrencada + recollida), no 120'. Si vas just, retalla primer: el **diccionari de transicions** de `maquina_estats_semafor.py` (deixa només la versió amb `if/elif` encadenats; el diccionari reapareix igualment com a variant al «Si t'encalles»).
