...
|---|---|---|---|---|
| **1** | Llegir sensors avançats i preparar-los per a la telemetria. | IMU MPU6050 (orientació/gestos), DHT11 (temperatura/humitat), BMP280 (pressió), CCS811 (CO₂): lectura i interpretació de magnituds al rover. `comportaments.py` s'estructura explícitament en **`percep()`/`decideix()`/`actua()`**, el mateix esquema que la plantilla del repte final de SA9. Disseny del format de missatge de telemetria (què s'envia i amb quina freqüència). *(Sessió fusionable amb la S2 en cas de compressió — vegeu pla de contingència del doc 08.)* | — | Simulador python.microbit.org: provar l'enviament d'un missatge de ràdio amb un valor numèric simulat. |
