...
- **Task 12 — SA6 Control** (4 sessions, S4 = prova T2): `termostat_histeresi` (LM35 + relé/LED, llaç tancat), `maquina_estats_semafor` (FSM amb dict/if), `registre_dades` (data logging natiu V2, lectura per USB), `vehicle_seguretat` (vehicle T2: teledirigit + aturada d'emergència per ultrasons — producte del trimestre). Fabricació vehicle segons `00_Projecte_T2_Vehicle.md`.
